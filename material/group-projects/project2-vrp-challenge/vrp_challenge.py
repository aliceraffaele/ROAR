...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  9 16:45:00 2022

@author: Gabriella Colajanni, Alessandro Gobbi, Alice Raffaele
"""
...
# python vrp_challenge.py nome_istanza
if __name__ == '__main__':
    nome_istanza = sys.argv[1]
    ...
    modello += lpSum(u[v]*costo_fisso_veicolo for v in V) + lpSum(x[i][j][v]*tempi[i][j] for i in S+C for j in S+C for v in V)*costo_benzina_minuto
    ...
    for v in V:
        modello += lpSum(y[c][v]*clienti[c]['num_borse'] for c in clienti) <= veicoli[v]['capacità']
